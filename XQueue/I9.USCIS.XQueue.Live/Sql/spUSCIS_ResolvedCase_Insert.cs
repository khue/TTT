...
namespace I9.USCIS.XQueue.Live.Sql {
    ...
    internal class spUSCIS_ResolvedCase_Insert : FdblSql {
        ...
        public int StartDataReader(Items.CaseInfo ci) {
            ...
            if (!string.IsNullOrEmpty(ci.ResponseCode))
            {
                SqlCommand.Parameters["@ResponseCode"].Value = ci.ResponseCode;
            }
            else if (!string.IsNullOrEmpty(ci.ResolutionCode))
            {
                SqlCommand.Parameters["@ResponseCode"].Value = ci.ResolutionCode;
            }
